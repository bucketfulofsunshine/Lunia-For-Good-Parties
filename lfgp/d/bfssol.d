...
~The charge laid against you is thus: the destruction of Saradush and the death of those trapped within its walls.~ 
...
== KELDO25J IF ~InParty("Keldorn") InMyArea("Keldorn") !StateCheck("Keldorn",CD_STATE_NOTVALID)~ THEN ~To be named in the Scroll of Retribution is no trivial matter. But you strive to overcome your dark heritage, <CHARNAME>, and I have taken your measure. I will stand beside you wherever this path leads.~
== ANOME25J IF ~InParty("Anomen") InMyArea("Anomen") !StateCheck("Anomen",CD_STATE_NOTVALID) Alignment("ANOMEN",LAWFUL_GOOD)~ THEN ~I would not wish such a punishment on anyone lightly. But, perhaps, this merely another trial. A final test to prove our worth in the eyes of the gods.~
== JAHEI25J IF ~InParty("Jaheira") InMyArea("Jaheira") !StateCheck("Jaheira",CD_STATE_NOTVALID)~ THEN ~Tyr's servants do not act without reason, but neither are they infallible. I suggest we solve this matter before we must fight a war on multiple fronts.~
== HAERD25J IF ~InParty("HaerDalis") InMyArea("HaerDalis") !StateCheck("HaerDalis",CD_STATE_NOTVALID)~ THEN ~It seems even the heavens have their dissonant notes. A word of advice, <CHARNAME>, I've performed in more forgiving courts than that one.~
...
== SAREV25J IF ~InParty("SAREVOK") InMyArea("SAREVOK") !StateCheck("SAREVOK",CD_STATE_NOTVALID) Alignment("SAREVOK",CHAOTIC_GOOD)~ THEN ~My past choices earn me a thousand damnations, but Saradush's destruction isn't one of them. Let us put this matter to rest, <CHARNAME> I refuse to carry guilt for a crime we did not commit.~
END
++ ~If justice has been twisted, then I will see it set right. Even if it means standing trial.~ EXTERN bfssol bfs.luniaquestaccept
++ ~Innocents *did* die, but not by my hand. I will act on their behalf, not yours.~ EXTERN bfssol bfs.luniaquestaccept
